...
const inputStyle = 'md:w-[70%]  w-[90%] text-right  p-2 outline-none border-b-2 border-gray-200'
...
setUserData({...userData , [e.target.name]:e.target.value})
  ...
  const updateUser = async() =>{
   const {data} = await usersUpdate({...userData , birthday:birthday} , user._id)
 ...
 localStorage.setItem(("user" ) ,JSON.stringify( {data :filterdUsers} ))
  ...
  return (
    <div className='w-[100%] md:h-[592px] pt-10 flex flex-col md:text-right text-center py-3 px-10 bg-white'>
        ...
        <input defaultValue={user?.name} onChange={handleChange} name='name' className={inputStyle} type="text" placeholder='שם' /></div>
        
        <div className=' flex flex-col items-end   mb-5'>
        <label className='my-1 font-medium' htmlFor="">דוא"ל</label>
        <input defaultValue={user?.email} onChange={handleChange} name='email'  className={inputStyle} type="email" placeholder='דואל' /></div>
        <div className=' flex flex-col items-end   mb-5'>
        <label className='my-1 font-medium' htmlFor="">תאריך לידה</label>
        <input  defaultValue={user?.birthday} onChange={(e)=>setBirthday(e.target.value)}  className={inputStyle} type="date" placeholder='' />
