...
setUserData({...userData , [e.target.name]:e.target.value})
  ...
  return (
    <div className='w-[100%] h-[100%] pt-10 flex flex-col md:text-right text-center py-3 px-10 bg-white'>
        ...
        <input defaultValue={user?.data?.name} onChange={handleChange} name='name' className={inputStyle} type="text" placeholder='שם' /></div>
        
        <div className=' flex flex-col items-end   mb-5'>
        <label className='my-1 font-medium' htmlFor="">דוא"ל</label>
        <input defaultValue={user?.data.email} onChange={handleChange} name='email'  className={inputStyle} type="email" placeholder='דואל' /></div>
        <div className=' flex flex-col items-end   mb-5'>
